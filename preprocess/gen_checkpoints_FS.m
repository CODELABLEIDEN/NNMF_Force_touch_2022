function gen_checkpoints_FS(unique_name,bandpass_upper, aligned, erp_data, options)
...
save_folder = sprintf('erp_%s%d',unique_name,bandpass_upper);
...
for i=1:ceil((137-checkpoint_n)/checkpoint_n)
    [files_grouped,A,parfor_time] = call_f_all_p_parallel_f(processed_data_path,f, 'start_idx', count, 'end_idx', count+checkpoint_n, 'aligned_taps',aligned, 'erp_data', erp_data, 'bandpass_upper',bandpass_upper, 'epoch_window_ms', options.epoch_window_ms, 'epoch_window_baseline', options.epoch_window_baseline, 'all', 0);
    save(sprintf('%s/check_%s%d_%d_to_%d.mat',save_path,unique_name, bandpass_upper,count, count+checkpoint_n), 'A', '-v7.3')
