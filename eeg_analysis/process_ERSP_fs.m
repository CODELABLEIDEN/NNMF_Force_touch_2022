function [data] = process_ERSP_fs(EEG_structs, participant, options, fs1s)
...
if ~options.erp_data
    equiv = options.fs1s(~cellfun('isempty', strfind(options.fs1s(:,1),participant)),:);
%     erp_ersp_data = equiv{:,4};
%     [chan] = sel_most_prevalent_chan(EEG_epoched_start,squeeze(trimmean(erp_ersp_data(:,1500:2100,:),20,3)),1);
    EEG_epoched_start.data(:,:,equiv{11}) = [];
    EEG_epoched_start.trials = size(EEG_epoched_start.data,3);
    EEG_epoched_start.epoch(equiv{11}) = [];
end
%% threshold EEG data
...
do_reject = 1;
try
    [~,idx_rej_trials_start] = pop_eegthresh(EEG_epoched_start, 1, electrodes_to_reject, voltage_lower_threshold, voltage_upper_threshold, start_time, end_time, do_superpose, do_reject);
    [~, idx_rej_trials_mid] = pop_eegthresh(EEG_epoched_mid, 1, electrodes_to_reject, voltage_lower_threshold, voltage_upper_threshold, start_time, end_time, do_superpose, do_reject);
catch ME
    return
end
...
% channel = chan;
            ...
            data{channel,7} = PA;
            data{channel,8} = idx_rej_trials_start;
        end
    end
end
end